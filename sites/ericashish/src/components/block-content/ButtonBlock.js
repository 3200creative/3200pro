...
function ButtonBlock({ parentProps, ...rest }) {
  const sanityProps = parentProps || rest
  return (
    <div
      sx={{
        width: '100%',
        margin: '0 auto',
        textAlign: sanityProps.position,
        ...
        >
          <Button
            sx={{
              ...
              variant: 'variants.buttons',
            }}
          >
            <div>{btn.buttonTxt}</div>
            {btn.icon && (
              <span
                sx={{
                  float: btn.iconPosition,
                  pl: (btn.iconPosition = 'left' ? 2 : 0),
                  pr: (btn.iconPosition = 'right' ? 2 : 0),
                }}
              >
                {Icons(btn.icon)}
              </span>
